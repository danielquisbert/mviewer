...
mviewer = (function () {
    ...
    var _queryMap = function (evt, options) {
        ...
        var featureInfosHtml = []; // structured html infos are put here
        ...
        if (queryType === 'map') {
            ...
            for(var layerid in vectorLayers) {                
                if (mviewer.hooks[layerid] && mviewer.hooks[layerid].handle) {
                    ...
                    id_tab+=1;
                    tabs.splice(l.rank,1,'<li title="'+l.name+'"><a href="#slide-'+id_tab+'" >'+l.name+'</a></li>');
                    var slide=$('<div id="slide-'+id_tab +'" ><ul class="bjqs"> </ul> </div>');                                    
            ...
            var callback = function (result) {               
                ...
                $.each(featureInfoByLayer, function (index, response) {
                    ...
                    {                        
                        ...
                        if (html) {                                                      
                            tabs.splice(response.layerinfos.rank,1,'<li title="'+name+'"><a href="#slide-'+id_tab+'" >'+name+'</a></li>');
                            slide =$('<div id="slide-'+id_tab +'" ><ul class="bjqs"> </ul> </div>'); 
    ...
    var _mouseOverFeature = function (evt) {
        ...
        var features = [];
        var hit = _map.forEachFeatureAtPixel(pixel, function (feature, layer) {
            return true;
        });
        if (hit) {
    ...
    var createContentHtml = function (features, olayer) {
        ...
        features.forEach(function (feature) {
            var nbimg = 0;
            counter+=1;            
            var li = '<li><div class="popupitem" style="padding-left: 25px;" ><h3>'+olayer.name+' : <font size=-1>' + feature.properties[olayer.fields[0]] +'</font></h3>';           
            ...
            fields.forEach(function (f) {
                if (attributes[f] && f != fields[0]) { // si valeur != null
                    if (attributes[f].indexOf("http://") >= 0) {                        
                        ...
                        li +=  getAlias(f, aliases, fields) + ' : ' + attributes[f] + '<br>';
                    }
                }
            });
            li += '</di></li>';            
    ...
    return {
        ...
        init: function (xml) {
           // try {
                ...
                if (applicationOverride.attr("panelstate") === "close" ) {
                    $("#themepanel").panel("close");
                } else {
                     $("#themepanel").panel("open");
                }
                if (applicationOverride.attr("showcoordinates") === "true" ) {
                    $("#mapinfos").prepend('<code id="hdms">position</code>');
                } 
                //hack css modif
        ...
        mapShare: function () {
            ...
            $("#themeroller").remove();
            $("#simpleshare").parent().append("<div id='themeroller' style='text-align: center;'><a href='http://themeroller.jquerymobile.com/?ver=1.4.5&style_id=20160106-14' title='Personnaliser le thème' target='_blank'><img src='img/logo/themeroller-mobile-logo.png' alt='themeroller'/></a></div>");
